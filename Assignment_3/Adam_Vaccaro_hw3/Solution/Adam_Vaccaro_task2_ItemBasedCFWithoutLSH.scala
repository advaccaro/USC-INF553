...
object ItemBasedCFWithoutLSH {
  def main(args: Array[String]): Unit = {
    ...
    val predictions = runPredictions(testDataPairs, productHashMap, userHashMap, userToProductsMap)//, corrHashMap)
  ...
  def runPredictions(testDataPairs: RDD[(Int, Int)],
    ...
    val predictions = testDataPairs.map(pair => {
      ...
      val k = 3
      val neighborhood = findNeighborhood(activeUser, productID, userToProductsMap, productHashMap)//, corrHashMap)
        .filter(!_._2.isNaN)
        .filter(_._2 != 0.0)
